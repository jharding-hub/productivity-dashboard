...
// CLOCK
function updateClock(){const now=new Date();const t=now.toLocaleTimeString('en-US',{hour12:true});const clk=document.getElementById('clock');if(clk)clk.textContent=t;const cd=document.getElementById('clockDate');if(cd)cd.textContent=now.toLocaleDateString('en-US',{weekday:'long',year:'numeric',month:'long',day:'numeric'});const mhc=document.getElementById('mobileHomeClock');if(mhc)mhc.textContent=t;if(!timerRunning&&timerLeft===timerTotal){var htl=document.getElementById('headerTimerLabel');if(htl)htl.innerHTML='<span class="header-timer-idle-icon" aria-hidden="true">&#9654;</span> Start focus';
/* PD gets the SHORT idle label. Its timer shares the nav row with the
   Dashboard button and the project dropdown (375px phones have ~335px to
   split), and "Start focus" was the one width that didn't fit -- the
   flex-wrap escape hatch dropped the whole timer to its own line (Joe,
   2026-08-25). A running timer ("24:58") fits, so only idle needed the
   trim. */
var pdl=document.getElementById('pdHeaderTimerLabel');if(pdl)pdl.innerHTML='<span class="header-timer-idle-icon" aria-hidden="true">&#9654;</span> Focus';}}
...
// R13 copy pass: was bare text with no way forward, the one panel of the
// four (Tasks/Notes/Projects/Reminders) still missing the message+button
// pattern the report asked for. Matches Tasks' "let the adjacent button
// carry the verb" phrasing rather than Projects' longer value line -- this
// panel doesn't need to sell itself, it needs to get out of the way.
if(visible.length===0){el.innerHTML='<div class="empty-state">'+(remSearch?'No matching reminders.':'<p style="margin:0 0 8px;color:var(--text-dim);">No reminders yet.</p><button class="btn btn-accent btn-sm" onclick="document.getElementById(\'newRemText\').focus()" style="margin:0 auto;display:block;">+ Add your first reminder</button>')+'</div>'+(remInOverlay?_renderRemindersArchiveSection():'');document.getElementById('remCount').textContent=sorted.filter(r=>!r.date||r.date>=today).length;if(typeof _updateTileSummaryReminders==='function')_updateTileSummaryReminders();return;}
...
function openProjMultiPicker(ev,picker){
  ...
  function positionDropdown(){
    ...
    var top=openUp?(rect.top-ddH-4):(rect.bottom+4);
    // Clamp into the viewport on BOTH branches. The anchor's viewport position
    // is NOT bounded by the window: panel overlays scroll their own content, so
    // a picker can sit at rect.top = 9357 or well above 0 while still being the
    // element the user just clicked. The open-down branch used rect.bottom+4
    // raw, which put the dropdown at top:-470px (jammed above the top edge --
    // Joe's "picker up high", 2026-09-01) or thousands of px below the fold.
    // Only the open-up branch had a floor, and it had no ceiling.
    var maxTop=window.innerHeight-ddH-8;
...
// R13 copy pass: was bare text, no way forward -- same gap Reminders had,
// same fix. Tab-scoped (morning/evening/custom all render through this),
// so the button focuses the always-visible add input rather than naming
// a specific tab in the copy.
function renderRoutines(){const tab=state.currentRoutineTab,items=state.routines[tab]||[];const el=document.getElementById('routineList');if(items.length===0){el.innerHTML='<div class="empty-state"><p style="margin:0 0 8px;color:var(--text-dim);">No routine items yet.</p><button class="btn btn-accent btn-sm" onclick="document.getElementById(\'newRoutineName\').focus()" style="margin:0 auto;display:block;">+ Add your first item</button></div>';}else{el.innerHTML=items.map(r=>'<div class="routine-item '+(r.done?'r-done':'')+'" data-rid="'+r.id+'" onclick="toggleRoutine(\''+tab+"','"+r.id+"',event)\""+' style="cursor:pointer"><div class="r-check '+(r.done?'r-checked':'')+'">'+(r.done?'\u2713':'')+'</div><span class="r-name editable" id="rn_'+r.id+'">'+esc(r.name)+'</span><span class="r-delete" onclick="event.stopPropagation();deleteRoutine(\''+tab+"','"+r.id+'\')">\u2715</span></div>').join('');}const done=items.filter(r=>r.done).length;document.getElementById('routineProgress').textContent=done+'/'+items.length;items.forEach(r=>{const e=document.getElementById('rn_'+r.id);if(e)makeEditable(e,v=>editRoutineName(tab,r.id,v));});refreshEditables();}
